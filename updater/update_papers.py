...
DOMAINS_YML = ROOT / "updater" / "domains.yml"
...
# Maximum items per domain keyword query (Crossref /works). Keep modest to reduce noise.
DOMAIN_QUERY_ROWS = 40
...
def load_domains() -> Dict[str, Dict[str, List[str]]]:
    """Load optional cross-domain query clusters from updater/domains.yml."""
    if not DOMAINS_YML.exists():
        return {}
    data = yaml.safe_load(DOMAINS_YML.read_text(encoding="utf-8")) or {}
    domains = data.get("domains", {}) or {}
    cleaned: Dict[str, Dict[str, List[str]]] = {}
    for name, cfg in domains.items():
        if not isinstance(cfg, dict):
            continue
        ident = cfg.get("identity_terms", []) or []
        ctx = cfg.get("context_terms", []) or []
        if not isinstance(ident, list) or not isinstance(ctx, list):
            continue
        cleaned[name] = {"identity_terms": ident, "context_terms": ctx}
    return cleaned
...
def fetch_recent_works_for_domain_query(
    domain_name: str,
    identity_terms: List[str],
    context_terms: List[str],
    from_date: str,
    rows: int = DOMAIN_QUERY_ROWS,
) -> List[Dict[str, Any]]:
    """Fetch recent Crossref works using an identity + context keyword query."""
    ident = " OR ".join(identity_terms)
    ctx = " OR ".join(context_terms)
    # Crossref 'query' is free-text. Using grouped terms improves recall but is not strict boolean.
    query = f"({ident}) ({ctx})"
    ...
    msg = crossref_get("works", params).get("message", {})
    items = msg.get("items", []) or []
    for it in items:
        it["_retrieval_stream"] = "domain_query"
        it["_retrieval_domain"] = domain_name
    return items
...
def main() -> None:
    ...
    # Optional: broaden coverage with cross-domain keyword queries across all journals
    domains_cfg = load_domains()
    if domains_cfg:
        for domain_name, cfg in domains_cfg.items():
            identity_terms = cfg.get("identity_terms", [])
            context_terms = cfg.get("context_terms", [])
            if not identity_terms or not context_terms:
                continue

            try:
                items = fetch_recent_works_for_domain_query(
                    domain_name=domain_name,
                    identity_terms=identity_terms,
                    context_terms=context_terms,
                    from_date=from_date,
                    rows=DOMAIN_QUERY_ROWS,
                )
            except Exception:
                continue
            ...
            for it in items:
                ...
                # Crossref abstracts are often JATS; reuse the existing cleaner
                if abstract:
                    abstract = clean_jats(abstract)

                text_for_rules = f"{title}\n{abstract}"

                # For cross-domain queries, apply the stricter tier behaviour (person-focused required)
                include, ann = apply_rules(text_for_rules, journal_tier="1B")
                if not include:
                    continue

                authors = format_authors(it.get("author") or [])
                container = it.get("container-title") or []
                journal_name = container[0] if container else ""

                published_date = extract_published_date(it)
                year = int(published_date[:4]) if published_date and published_date[:4].isdigit() else None

                url = doi_url(doi)
                neurotype = classify_neurotype(text_for_rules)
                ...
                paper_obj: Dict[str, Any] = {
                    ...
                    "domains": ann.get("domains", []),
                    "tags": ann.get("tags", []),
                    "retrieval_stream": it.get("_retrieval_stream", "domain_query"),
                    "retrieval_domain": it.get("_retrieval_domain", domain_name),
                }
    ...
    out = {
        ...
        "source_summary": "Updated daily from Crossref using a journal watchlist plus person-focused filters, plus optional cross-domain keyword queries",
        "schema_version": "1.0",
        "papers": papers_out,
    }
