...
# Maximum items per domain keyword query (Crossref /works). Keep modest to avoid noise.
DOMAIN_QUERY_ROWS = 60
...
def load_domains() -> Dict[str, Dict[str, List[str]]]:
    """Load optional domain query configuration.

    If updater/domains.yml does not exist, returns an empty dict and the updater
    will operate in journal-watchlist mode only.
    """
    if not DOMAINS_YML.exists():
        return {}
    raw = yaml.safe_load(DOMAINS_YML.read_text(encoding="utf-8")) or {}
    ...
    for name, cfg in domains.items():
        if not isinstance(cfg, dict):
            continue
        identity = cfg.get("identity_terms") or []
        context = cfg.get("context_terms") or []
        if not identity or not context:
            continue
        out[str(name)] = {
            "identity_terms": [str(x).strip() for x in identity if str(x).strip()],
            "context_terms": [str(x).strip() for x in context if str(x).strip()],
        }
...
def fetch_recent_works_for_domain_query(
    ...
    """Fetch recent works from Crossref across all journals using a domain query.

    This broadens coverage beyond the journal watchlist and supports NeuroPeople as a
    cross-domain evidence map.
    """
    identity_query = " OR ".join(identity_terms)
    context_query = " OR ".join(context_terms)
    query = f"({identity_query}) AND ({context_query})"

    filt = f"from-pub-date:{from_date},type:journal-article"
    data = crossref_get(
        "/works",
        {
        ...
        for issn in issns:
            ...
            for item in works:
                ...
                paper_obj: Dict[str, Any] = {
                    ...
                    "tags": ann.get("tags", []),
                    "retrieval_stream": item.get("_retrieval_stream", "journal_watchlist"),
                    "retrieval_domain": item.get("_retrieval_domain"),
                }
...
# Optional: broaden coverage with domain keyword queries across all journals
if domains_cfg:
    for domain_name, cfg in domains_cfg.items():
        try:
            ...
            # Keep going if a single query fails
            continue

        # Process items using Tier 1A rules (neuro + person-focused OR not mechanistic)
        for item in items:
            title = extract_title(item)
            abstract = extract_abstract(item)
            journal_name = extract_journal(item)
            authors = extract_authors(item)
            year, published_date = extract_year_and_date(item)
            ...
            include, ann = apply_rules(blob, journal_tier="1A")
            if not include:
                continue
            ...
            paper_obj: Dict[str, Any] = {
                ...
                "domains": ann.get("domains", []),
                "tags": ann.get("tags", []),
                "retrieval_stream": item.get("_retrieval_stream", "domain_query"),
                "retrieval_domain": item.get("_retrieval_domain"),
            }
    ...
    out = {
        ...
        "source_summary": "Updated daily from Crossref using a journal watchlist plus optional cross-domain keyword queries",
        "schema_version": "1.0",
        "papers": papers_out,
    }
